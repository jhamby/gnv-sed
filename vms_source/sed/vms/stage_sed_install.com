...
$   create/dir new_gnu:[usr.share.doc.sed]/prot=o:rwed
...
$ if mode .eqs. "install"
$ then
$    copy [.vms]gnv_sed_startup.com -
         new_gnu:[vms_bin]gnv$sed_startup.com
$ else
$    file = "new_gnu:[vms_bin]gnv$sed_startup.com"
...
$!
$!   If gnv$xxx.exe, then:
$!       Source is []gnv$sed.exe
$!       Destination1 is new_gnu:[bin]gnv$sed.exe
...
$!       dest is [usr.share.doc.sed]
$   if f$locate(".doc.sed]", tdir) .lt. tdir_len
$   then
$       source = "sys$disk:[]''tname'''ttype'"
...
$           source = "sys$disk:[]$''tname'''ttype'"
$       endif
$       dest = "new_gnu:[usr.share.doc.sed]''tname'''ttype'"
$       if mode .eqs. "install"
$       then
$           if f$search(dest) .eqs. "" then copy 'source' 'dest'
$       else
